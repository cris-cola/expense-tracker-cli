import { readExpensesFromJson } from "../store";
import { Expense } from "../types";
import { consoleError, consoleInfo } from "../utils";

function getExpensesByMonth(month: string, expenses: Expense[]) {
  const monthIndex = parseInt(month, 10);
  if(!isNaN(monthIndex)) return expenses.filter(x => (x.createdAt as Date).getMonth() === monthIndex - 1 );
...
export function getExpensesSummary(month?: string) {
